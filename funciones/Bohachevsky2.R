...
evaluadora<- function(poblacion)
{
  ...
  # Bohachevsky no. 2 function adpated by P. Mateo from codes in ww.sfu.ca/~ssurjano/index.html
  ...
  if(is.matrix(poblacion)  ){
    ...
    for(j in 1:d[1])
    {
      ...
      term3 <- -0.3 * cos(3*pi*poblacion[j,1])* cos(4*pi*poblacion[j,2])      
      f[j] <- term1 + term2 + term3 + 0.3
      f[j] <- -f[j]
    }
  }
  else 
